...
import { CLIENT_STATUSES, type ClientStatus } from "@/lib/status";

export function ClientStatusPipeline({
  clientId,
  status,
  canWrite,
}: {
  ...
  async function setStatus(next: ClientStatus) {
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-white/[0.02] p-4">
      ...
      <ol className="flex min-w-max items-center gap-0 overflow-x-auto">
        {CLIENT_STATUSES.map((step, index) => {
          const active = step === current;
          const reached = CLIENT_STATUSES.indexOf(current) >= index;
          return (
