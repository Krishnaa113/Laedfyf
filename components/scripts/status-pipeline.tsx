...
import { SCRIPT_STATUSES, type ScriptStatus } from "@/lib/status";

export function ScriptStatusPipeline({
  scriptId,
  status,
  canWrite,
}: {
  ...
  const [current, setCurrent] = useState<ScriptStatus>(status);
  const [error, setError] = useState<string | null>(null);
  const [pending, setPending] = useState(false);

  async function setStatus(next: ScriptStatus) {
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-white/[0.02] p-4">
      ...
      <ol className="flex min-w-max items-center gap-0 overflow-x-auto">
        {SCRIPT_STATUSES.map((step, index) => {
          const active = step === current;
          const reached = SCRIPT_STATUSES.indexOf(current) >= index;
          return (
            <li key={step} className="flex items-center">
              ...
              {index < SCRIPT_STATUSES.length - 1 ? (
                <span
